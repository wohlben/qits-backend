...
- **`artifactory/`** — the metadata-rich blob store (`docs/epics/qits-artifactory/`) as a plain library jar (`eu.wohlben.qits.artifactory`, BCE split + framework-free `error/`). Content-addressed immutable blobs (SHA-256) + a flat string metadata map + typed repositories (`ci-screenshots`/`ci-videos`) + a query API with a `latest` collapse. Depends on **neither `domain` nor `auth/*`** (blobs reference branches/flows by string metadata, never FK), and owns its **own named datasource + persistence unit + Flyway lineage** (a separate H2 under `~/.qits/data/artifactory`, migrations at `db/artifactory/migration`), so a future split into a standalone deployable moves files, not data. Config defaults ship in its `META-INF/microprofile-config.properties`; `service` indexes it via `quarkus.index-dependency.artifactory.*` and hosts its REST boundary (`eu.wohlben.qits.artifactory.api`, all `/api/artifactory`, hidden from OpenAPI). Writes are guarded by a static `qits.artifactory.token` (blank ⇒ open dev/test); reads are open (usable as `<img>` src). `-pl artifactory` never needs `-Dqits.variant`.
- **`service/`** — the Quarkus web app: REST controllers (`<area>.api`), exception mappers (`eu.wohlben.qits.api`), `mutiny`, `health`, and the Angular UI (Quinoa). Depends on `domain` + `artifactory` + exactly one auth variant module (below). Contains **no auth code** itself.
