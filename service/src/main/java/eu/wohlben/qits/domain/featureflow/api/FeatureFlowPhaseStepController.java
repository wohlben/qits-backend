...
public class FeatureFlowPhaseStepController {
    ...
    @POST
    public CreateFeatureFlowPhaseStepRequest.Response create(@Valid CreateFeatureFlowPhaseStepRequest request) {
        var step = featureFlowPhaseStepService.create(
            request.phaseId(), request.name(), request.sortOrder()
        );
        return new CreateFeatureFlowPhaseStepRequest.Response(
            featureFlowPhaseMapper.toStepDto(step)
        );
    }
    ...
    public GetFeatureFlowPhaseStepRequest.Response get(@PathParam("id") String id) {
        ...
        return new GetFeatureFlowPhaseStepRequest.Response(
            featureFlowPhaseMapper.toStepDto(step)
        );
    }
    ...
    public ListFeatureFlowPhaseStepsRequest.Response list(@QueryParam("phaseId") String phaseId) {
        List<eu.wohlben.qits.domain.featureflow.entity.FeatureFlowPhaseStep> steps = phaseId != null
            ? featureFlowPhaseStepService.listByPhase(phaseId)
            : featureFlowPhaseStepService.listAll();
        var entries = steps.stream()
            .map(s -> new ListFeatureFlowPhaseStepsRequest.Response.Entry(
    ...
    @PUT
    @Path("/{id}")
    public UpdateFeatureFlowPhaseStepRequest.Response update(@PathParam("id") String id, @Valid UpdateFeatureFlowPhaseStepRequest request) {
        var step = featureFlowPhaseStepService.update(
            id, request.name(), request.sortOrder()
        );
        return new UpdateFeatureFlowPhaseStepRequest.Response(
            featureFlowPhaseMapper.toStepDto(step)
        );
    }
