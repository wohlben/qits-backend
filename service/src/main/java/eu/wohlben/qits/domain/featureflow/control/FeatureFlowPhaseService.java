...
public class FeatureFlowPhaseService {
    ...
    @Transactional
    public FeatureFlowPhase create(String featureFlowConfigurationId, String name, String description, int orderIndex, String parentPhaseId) {
        ...
        FeatureFlowConfiguration config = featureFlowConfigurationRepository.findByIdOptional(featureFlowConfigurationId)
            .orElseThrow(() -> new NotFoundException("FeatureFlowConfiguration not found: " + featureFlowConfigurationId));
        ...
        if (parentPhaseId != null && !parentPhaseId.isBlank()) {
            FeatureFlowPhase parent = featureFlowPhaseRepository.findByIdOptional(parentPhaseId)
                .orElseThrow(() -> new NotFoundException("Parent FeatureFlowPhase not found: " + parentPhaseId));
            if (!parent.featureFlowConfiguration.id.equals(config.id)) {
                throw new BadRequestException("Parent phase must belong to the same FeatureFlowConfiguration");
            }
    ...
    public FeatureFlowPhase get(String id) {
        return featureFlowPhaseRepository.findByIdOptional(id)
    ...
    @Transactional
    public FeatureFlowPhase update(String id, String name, String description, Integer orderIndex, String parentPhaseId) {
        FeatureFlowPhase phase = featureFlowPhaseRepository.findByIdOptional(id)
            .orElseThrow(() -> new NotFoundException("FeatureFlowPhase not found: " + id));
        ...
        if (parentPhaseId != null) {
            if (parentPhaseId.isBlank()) {
                ...
                if (wouldCreateCycle(phase, parent)) {
                    throw new BadRequestException("Setting this parent would create a cycle in the phase hierarchy");
                }
                phase.parentPhase = parent;
            }
        }
