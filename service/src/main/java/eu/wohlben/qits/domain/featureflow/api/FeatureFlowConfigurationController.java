  ...
  public ListFeatureFlowConfigurationsRequest.Response list() {
    ...
    var entries =
        configs.stream()
            .map(
                c ->
                    new ListFeatureFlowConfigurationsRequest.Response.Entry(
                        featureFlowConfigurationMapper.toDto(c)))
            .toList();
    return new ListFeatureFlowConfigurationsRequest.Response(entries);
  }

  public static record UpdateFeatureFlowConfigurationRequest(@NotBlankIfPresent String name) {
    public record Response(FeatureFlowConfigurationDto featureFlowConfiguration) {}
  }

  @PUT
  @Path("/{id}")
  public UpdateFeatureFlowConfigurationRequest.Response update(
      @PathParam("id") String id, @Valid UpdateFeatureFlowConfigurationRequest request) {
