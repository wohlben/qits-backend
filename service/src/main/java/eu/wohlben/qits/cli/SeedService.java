...
public class SeedService {
  ...
  /**
   * Resolves the fixture path. Maven goals run with the working dir at the {@code service} module,
   * but fall back to the repo-root layout so it also works if launched from there.
   */
  private String resolveRepoUrl() {
    ...
    String[] candidates = {
      ...
      "service/src/test/resources/fixtures/testing-repo.git",
    };
    ...
    throw new IllegalStateException(
        "testing-repo.git fixture not found (cwd=" + Path.of("").toAbsolutePath() + ")");
  }
}
