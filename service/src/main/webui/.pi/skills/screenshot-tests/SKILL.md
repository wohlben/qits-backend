...
**Producing toolchain (baseline provenance).** The committed baselines are only reproducible on the exact browser build **and font stack** that rendered them, so both are baked into the `qits/workspace` image (which the devcontainer extends) — that image is the **sole sanctioned producer of baselines**. The renderer is pinned twice, and the pins must agree: `pnpm-lock.yaml` resolves `playwright` (currently `1.61.0` → Chromium revision 1228 = **Google Chrome for Testing 149.0.7827.55**), and `ARG PLAYWRIGHT_VERSION` in `docker/workspace/Dockerfile` bakes that same version's Chromium plus a fixed font package set into the image. The exact baked toolchain (Chromium build + every font package) is readable from inside any container: `cat /etc/qits-renderer-provenance`. All current baselines were re-recorded on the baked image (2026-07-13). Bumping `playwright` in `package.json`/`pnpm-lock.yaml` **requires** bumping the Dockerfile ARG, rebuilding the image (`docker build -t qits/workspace docker/workspace`), rebuilding the devcontainer on top of it, and re-recording the baselines — one reviewed change. Forgetting the rebuild fails loudly with `Executable doesn't exist at /opt/ms-playwright/…` — that error means **rebuild the image**, never "run `playwright install` locally" (a local install reintroduces an unsanctioned renderer). See [`docs/epics/qits-build-setup/features/2026-07-13_screenshot-baseline-renderer-baked-into-image.md`](../../../../../../../docs/epics/qits-build-setup/features/2026-07-13_screenshot-baseline-renderer-baked-into-image.md).
...
| Passes locally, fails on another machine / CI | **Time/locale-dependent rendering.** `DatePipe` formats in the machine's timezone, so a fixed ISO date can render different clock text. Keep fixtures timezone-stable and avoid any runtime clock/locale in what you capture. (Font-stack drift is no longer a suspect: fonts + Chromium are baked into the `qits/workspace` image — but only if the other machine actually runs that image. `Executable doesn't exist at /opt/ms-playwright/…` there means its image is stale: rebuild it, don't `playwright install`.) |
