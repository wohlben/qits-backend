...
describe('RepositorySyncComponent', () => {
  function setup() {
    const repositoryService = {
      ...
      apiRepositoriesRepoIdSyncPost: vi.fn().mockReturnValue(of({})),
  ...
  it('invalidates the repository synchronously on sync success (unchanged path)', async () => {
    const { component, dialog, invalidateSpy } = setup();

    component.syncMutation.mutate();
    await flush();

    expect(invalidateSpy).toHaveBeenCalledTimes(1);
    // Sync stays synchronous — no process dialog.
