...
import { WorkspaceHistoryDto } from '@/api/model/workspaceHistoryDto';
 ...
 */
@Component({
  selector: 'app-workspace-history-list',
  ...
  template: `
    @if (historyQuery.isPending()) {
      ...
      <app-empty-state>
        <span title>No workspace history yet</span>
        <span description>Workspaces you create, integrate or abandon will be recorded here.</span>
      </app-empty-state>
    } @else {
          ...
          <button class="text-left" (click)="open(wt)">
            <app-card-layout [hasActions]="false">
              <div cardTitle class="flex items-center gap-2">
                <h3 class="font-mono font-semibold">{{ wt.workspaceId }}</h3>
...
export class WorkspaceHistoryListComponent {
  ...
  readonly historyQuery = injectQuery(() => ({
    queryKey: ['workspace-history', this.repoId()],
    queryFn: () =>
      lastValueFrom(this.historyService.apiRepositoriesRepoIdHistoryGet(this.repoId())).then(
        (r) => r.entries?.map((e) => e.workspace!).filter((w): w is WorkspaceHistoryDto => !!w) ?? [],
      ),
  }));

  open(wt: WorkspaceHistoryDto) {
    if (wt.id != null) {
