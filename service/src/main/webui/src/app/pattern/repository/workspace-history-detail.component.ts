...
import { WorkspaceHistoryControllerService } from '@/api/api/workspaceHistoryController.service';
import { WorkspaceStatus } from '@/api/model/workspaceStatus';
...
/**
 * A single workspace's full history: its narrative (preamble/result markdown), the event timeline, and
 * the commands that ran in it (each linking to its terminal/log). Keyed by the surrogate id.
 */
@Component({
  selector: 'app-workspace-history-detail',
  ...
  template: `
    @if (detailQuery.isPending()) {
      ...
      <div class="py-12 text-center text-destructive">Failed to load workspace history</div>
    } @else if (detailQuery.data(); as wt) {
      <div class="flex flex-col gap-6">
        <h2 class="font-mono text-xl font-semibold">{{ wt.workspaceId }}</h2>
        ...
        <section class="flex flex-col gap-2">
          ...
          @if ((wt.commands ?? []).length) {
            ...
            <p class="text-sm text-muted-foreground">No commands ran in this workspace.</p>
          }
        </section>
      </div>
    }
...
export class WorkspaceHistoryDetailComponent {
  readonly repoId = input.required<string>();
  readonly workspaceRowId = input.required<number>();

  private readonly historyService = inject(WorkspaceHistoryControllerService);

  readonly detailQuery = injectQuery(() => ({
    queryKey: ['workspace-history-detail', this.repoId(), this.workspaceRowId()],
    queryFn: () =>
      lastValueFrom(
        this.historyService.apiRepositoriesRepoIdHistoryIdGet(this.workspaceRowId(), this.repoId()),
      ).then((r) => r.workspace ?? null),
  }));

  badgeType(status: WorkspaceStatus | undefined): ZardBadgeTypeVariants {
    switch (status) {
  ...
  statusLabel(status: WorkspaceStatus | undefined): string {
