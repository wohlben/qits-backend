...
import type { WorkspaceFileContentDto } from '@/api/model/workspaceFileContentDto';
 ...
 * workspace's content/structure and recomputed whenever that changes, so they live in their own list
...
/** One level of a workspace's file tree: eager files plus lazily-resolvable directory stubs. */
interface LazyLevel {
  paths: string[];
  lazyDirs: LazyDirDto[];
}
 ...
 */
@Component({
  selector: 'app-workspace-file-browser',
  imports: [
    ...ZardTreeImports,
    ...ZardSelectImports,
...
export class WorkspaceFileBrowserComponent {
   ...
   * {@code ['workspace-files', repoId, workspaceId, dir]} — a distinct cache entry per directory, so
  ...
  private readonly lazyListingsQuery = injectQueries(() => ({
    queries: this.openedLazyPaths().map((dir) => ({
      queryKey: ['workspace-files', this.repoId(), this.workspaceId(), dir],
  ...
  /** Distinct ignore-file basenames (`.gitignore`, `.dockerignore`, …) present in the workspace. */
  ...
  readonly fileQuery = injectQuery(() => ({
    queryKey: ['workspace-file', this.repoId(), this.workspaceId(), this.selectedPath()],
