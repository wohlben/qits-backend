 ...
 *
 * <p>{@code git worktree add} runs with its working directory set to the bare origin.
 * When {@code qits.repositories.data-dir} is a <em>relative</em> path (as in dev), a
 * relative worktree path would be created nested under origin instead of the repo's
 * worktrees directory — leaving {@code list}/{@code merge}/{@code discard} unable to
 * find it on disk. The other controller tests use an absolute temp dir and so never
 * exercised this case. This test pins the relative-data-dir behaviour.
 */
@QuarkusTest
...
public class WorktreeRelativeDataDirTest {
    ...
    private String createProjectAndRepository() {
        String projectId = given()
            ...
            .body(new eu.wohlben.qits.domain.project.api.ProjectController.CreateProjectRequest("Rel Worktree Project", null))
        .when()
            .post("/api/projects")
        .then()
    ...
    public void testFullLifecycleWithRelativeDataDir() {
        ...
        .then()
            .statusCode(Response.Status.OK.getStatusCode())
            .body("entries.find { it.worktree.worktreeId == 'rel-01' }.worktree.branch", equalTo("rel-branch"));
