...
public class MutinySmokeTest {
  ...
  public void uniFailureRecovery() {
    String result =
        Uni.createFrom()
            .<String>failure(new RuntimeException("boom"))
            .onFailure()
  ...
  public void uniCombineAll() {
    ...
    String result =
        Uni.combine().all().unis(a, b).with((x, y) -> x + y).await().atMost(Duration.ofSeconds(5));
  ...
  public void uniInvokeSideEffectDoesNotChangeItem() {
    ...
    String result =
        Uni.createFrom().item("x").invoke(sb::append).await().atMost(Duration.ofSeconds(5));
