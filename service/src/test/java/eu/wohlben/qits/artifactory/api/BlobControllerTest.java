...
class BlobControllerTest {
  ...
  void ensureRepo() {
    given()
        ...
        .when()
        .put("/api/artifactory/repositories/ci-screenshots")
        .then()
        .statusCode(200);
  }
  ...
  private io.restassured.specification.RequestSpecification upload(
    ...
    return given()
        ...
        .headers(ArtifactoryTestMedia.screenshotHeaders(branch, flow, 100, 50))
        .body(png);
  }
  ...
  void acceptsUploadsLargerThanThe10mDefaultWireLimit() {
    ...
    byte[] big = Arrays.copyOf(ArtifactoryTestMedia.png(100, 50, 99), 11 * 1024 * 1024);
    upload("main", "big", big)
        .when()
        .post("/api/artifactory/repositories/ci-screenshots/blobs")
        .then()
        .statusCode(201);
  }
  ...
  void uploadServeAndQueryRoundTrip() {
    byte[] png = ArtifactoryTestMedia.png(100, 50, 1);
    String id =
    ...
    byte[] served =
        given()
            .when()
            .get("/api/artifactory/repositories/ci-screenshots/blobs/" + id)
            .then()
            .statusCode(200)
            .contentType("image/png")
    ...
    given()
        ...
        .when()
        .get("/api/artifactory/repositories/ci-screenshots/blobs")
        .then()
        .statusCode(200)
  ...
  void sniffOverridesALyingContentTypeHeader() {
    byte[] png = ArtifactoryTestMedia.png(100, 50, 2);
    String id =
        given()
  ...
  void missingRequiredMetadataKeyIs400() {
    var headers = ArtifactoryTestMedia.screenshotHeaders("main", "checkout", 100, 50);
    headers.remove("X-Artifactory-Meta-qits.diff.hash");
    given()
        .contentType("image/png")
        .headers(headers)
        .body(ArtifactoryTestMedia.png(100, 50, 3))
        .when()
        .post("/api/artifactory/repositories/ci-screenshots/blobs")
        .then()
        .statusCode(400);
  ...
  void uploadToUnknownRepositoryIs404() {
    upload("main", "checkout", ArtifactoryTestMedia.png(100, 50, 4))
        .when()
        .post("/api/artifactory/repositories/nope/blobs")
        .then()
        .statusCode(404);
  }
  ...
  void serveMalformedIdIs404NotATraversal() {
    given()
        .when()
        .get("/api/artifactory/repositories/ci-screenshots/blobs/not-a-sha")
        .then()
        .statusCode(404);
  }
}
