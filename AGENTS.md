...
- **`artifacts/`** — the metadata-rich blob store (`docs/epics/qits-artifacts/`) as a plain library jar (`eu.wohlben.qits.artifacts`, BCE split + framework-free `error/`). Content-addressed immutable blobs (SHA-256) + a flat string metadata map + typed repositories (`ci-screenshots`/`ci-videos`) + a query API with a `latest` collapse. Depends on **neither `domain` nor `auth/*`** (blobs reference branches/flows by string metadata, never FK), and owns its **own named datasource + persistence unit + Flyway lineage** (a separate H2 under `~/.qits/data/artifacts`, migrations at `db/artifacts/migration`), so a future split into a standalone deployable moves files, not data. Config defaults ship in its `META-INF/microprofile-config.properties`; `service` indexes it via `quarkus.index-dependency.artifacts.*` and hosts its REST boundary (`eu.wohlben.qits.artifacts.api`, all `/api/artifacts`, hidden from OpenAPI). Writes are guarded by a static `qits.artifacts.token` (blank ⇒ open dev/test); reads are open (usable as `<img>` src). `-pl artifacts` never needs `-Dqits.variant`.
- **`ci/`** — the CI pipeline domain (`docs/epics/qits-ci/`) as a plain library jar (`eu.wohlben.qits.ci`, BCE split + framework-free `error/`), on the artifacts template: depends on **neither `domain` nor `auth/*`**, references repos/branches by string id (never FK), and owns its **own named datasource + persistence unit + Flyway lineage** (a separate H2 under `~/.qits/data/ci`, migrations at `db/ci/migration`). A repo opts in by committing `.config/qits/ci-post-receive.yml` (a `steps` list of `image` + `script`); the git host's post-receive hook POSTs each updated branch ref to the token-guarded intake (`qits.ci.token`, blank ⇒ open dev/test), and ci fetches the pushed commit with its own `git`, runs each step as a fresh container on qits-net via its own docker shell-out, and records per-step pass/fail. `service` indexes it via `quarkus.index-dependency.ci.*` and hosts its REST boundary (`eu.wohlben.qits.ci.api`, all `/api/ci`, hidden from OpenAPI). Only `/api/ci/events/` is on `PublicPaths` — run reads carry build logs, so they stay behind the auth policy. Because a step's script is repo-controlled code reached via the **unauthenticated** git host, step containers drop privileges and are capped (`qits.ci.{memory-limit,pids-limit,cpus}`) and the residual trust gap is tracked in `docs/issues/2026-07-26_ci-executes-repo-controlled-code-from-unauthenticated-pushes.md`. `-pl ci` never needs `-Dqits.variant`.
- **`service/`** — the Quarkus web app: REST controllers (`<area>.api`), exception mappers (`eu.wohlben.qits.api`), `mutiny`, `health`, and the Angular UI (Quinoa). Depends on `domain` + `artifacts` + exactly one auth variant module (below). Contains **no auth code** itself.
- **`auth/core`, `auth/oidc`, `auth/forwardauth`, `auth/local`** — build-variant auth as library jars (`docs/epics/qits-authentication/features/2026-07-16_build-variant-auth.md`). **Every service build that produces an artifact must name a variant with `-Dqits.variant=forwardauth|oauth|local`** (maven-enforcer at `prepare-package` fails flagless `package`/`install`/`verify`). The auth-scheme variants `forwardauth`/`oauth` **never run unauthenticated**; `auth/local` (`-Dqits.variant=local`) is the *explicitly* open variant — every request is a fixed local user, in any launch mode — for **trusted local starts only** (a workspace's own packaged qits behind the parent on qits-net; never internet-exposed). flagless **pre-packaging** work — `quarkus:dev`, `test` — defaults to forwardauth via the `variant-default-forwardauth` profile (activated on `!qits.variant`), i.e. effectively no auth in dev — the matching profile in `service/pom.xml` pulls in `auth-forwardauth` (trust forward-auth proxy headers; the everyday dev/test variant — its `%dev`/`%test` fallback identity `dev` keeps dev mode and the test suites friction-free) or `auth-oidc` (built-in Keycloak OIDC; the service test suite does NOT run under this variant — its coverage lives in `auth/oidc`'s own tests). `auth-core` (the always-on `QitsAuthPolicy`, the `PublicPaths` token-free list, `/api/auth/me`) arrives transitively. The auth modules ship config defaults in `META-INF/microprofile-config.properties` (read from dependency jars, unlike application.properties) and are bean-discovered via jandex-maven-plugin indexes, not `quarkus.index-dependency`.
- **`cli/`** — a Quarkus command-mode app (`@QuarkusMain` in `eu.wohlben.qits.cli.Main`) with no web stack. Depends on `domain` (no auth — the variant flag is never needed for cli-only commands). Exposes `seed` (`SeedService`, the tiny `testing-repo` demo), `seed-webapp` (`SeedWebappService`, the servable Quarkus+Angular demo — idempotent by reset), `seed-lit` (`SeedLitService`, the servable Quarkus+Lit web-components demo — idempotent by reset), and `generate-migration`.
...
# the real login wall.
#
# NOTE: to actually exercise workspace containers / the service web view, run this INSIDE the
...
# NOTE on memory: Quarkus pins one full app per @TestProfile/@WithTestResource class for a fork's
# lifetime (quarkusio/quarkus#38774), so the domain/service suites run CHUNKED: the default surefire
# execution runs all profile-less classes against one shared app, and 15 isolated-tests-N executions
# run the profile classes 2-3 per fresh fork (lists generated by scripts/list-isolated-tests.sh; caps
# in the root pom surefire argLine). A metaspace OOME in a chunk means too many new profiles landed
# in one chunk — bump the chunk count in domain/service poms + the script arg, don't raise the caps.
# Never pass -DargLine=... (the root pom's empty argLine property default shadows it — it silently
# never reaches the fork); use JAVA_TOOL_OPTIONS for ad-hoc fork flags. See
# docs/issues/resolved/2026-07-25_quarkus-test-metaspace-oom.md.

# Run a single test class / method (-am also builds the deps it needs). -Dtest=... collapses the
# chunked executions back to one plain execution (root pom single-test-override profile).
...
# Seed the servable Quarkus+Lit web-components demo: same shape as seed-webapp, but around the
# testing-repo-quarkus-lit fixture (multiple self-contained Lit components built and served through
# one Quinoa/Vite, web view on the Vite dev server :5173, framework kind `ts-lit`, `.test.ts`
# test↔code linking). Also idempotent by RESET.
./mvnw -pl cli quarkus:run -Dcli.args=seed-lit
...
Every workspace executes inside a **per-workspace Docker container** — the sole execution environment for action scripts, dev servers, services, and the coding agent (no host-execution fallback). See `docs/epics/qits-workspaces/features/2026-07-04_workspace-containers.md`.
...
- **Seeding is pure host-side data setup** (rows, branch refs, host-side worktree merges — services/actions come from the fixture's `.qits-config.yml`, read in-container on first use): the `cli` `seed`/`seed-webapp`/`seed-lit` commands run standalone, with no docker and no running service. Docker + the git server are needed only when a container actually materializes — on first use, via `ensureContainer`.
...
- **`testing-repo-quarkus-angular`** *(submodule → `wohlben/qits-fixture-quarkus-angular`, derived bare `testing-repo-quarkus-angular.git`)* — a minimal but **servable** Quarkus 3 + Angular app (`POST /api/greetings` echoing `{name, timestamp}` + an Angular SPA served by Quinoa), shaped like qits itself, for demoing features that run real work in a workspace (dev-server services, actions, the coding agent). Its `src/main/webui` is itself a **nested submodule → `qits-fixture-angular`** (relative url `../qits-fixture-angular.git`); qits imports it as a sibling repository and materializes it offline on qits-net. The SPA is the reference implementation of the SPA-observability convention: `GET /api/config.json` identity relay, `POST /api/otel/v1/*` OTLP passthrough, and an SPA instrumented via the **`@qits/angular` library** (a SHA-pinned git dependency on `github.com/wohlben/qits-angular-integration` — the fixture is its reference consumer; `docs/epics/qits-observability/features/2026-07-06_spa-observability.md`, `docs/epics/qits-integration-angular/features/2026-07-13_qits-angular-integration-library.md`), including a `GreetingHistoryStore` (`@ngrx/signals`) tagged `withQitsSnapshot('greetingHistory')` so captures carry app state (`docs/epics/qits-integration-angular/features/2026-07-14_capture-state-snapshot.md`). Branches: `main`, `feature/greeting` (FF gitlink advance over `main`), `feature/diverged` (gitlink conflict) — the greeting divergence now rides the `webui` submodule pointer, backend text identical. Build/run it with its own committed `./mvnw` (JDK 25 + pnpm) after `git submodule update --init`; it is **not** part of the qits Maven build. See `docs/epics/qits-testing-fixtures/features/2026-07-05_servable-quarkus-angular-fixture.md`. **Editing round-trip is two-level**: commit+push in `qits-fixture-angular` → bump the `webui` gitlink in `qits-fixture-quarkus-angular` → bump that gitlink in qits.
- **`testing-repo-quarkus-lit`** *(submodule → `wohlben/qits-fixture-quarkus-lit`, derived bare `testing-repo-quarkus-lit.git`)* — a minimal but **servable** Quarkus 3 + **Lit web-components** app (same `POST /api/greetings` backend + telemetry-gateway classes as the quarkus-angular fixture, but the webui is an **in-tree Vite + Lit + TypeScript** workspace — no nested submodule): one Rollup entry per self-contained component (`src/components/<name>/` → stable unhashed `dist/components/<name>.js`, sub-components registered only by their owner), a demo `index.html` composing `<qits-greeting-form>` + `<qits-greeting-history>` via a bubbling `qits-greeting` CustomEvent, base-relative API resolution (`document.baseURI`, optional `api-base` attribute), runnable vitest tests (`*.test.ts`, happy-dom), and a `.qits-config.yml` declaring the web-viewable `quarkus:dev` service (web view on Vite's :5173) + actions + the `ts-lit` framework kind. Branches: `main`, `feature/greeting` (FF, welcome note in `greeting-form/`), `feature/diverged` (real text conflict on the greeting-form heading). Consumed by `seed-lit`; detection-wise it lights the fourth descriptor `ts-lit` (Vite-config marker + lit-dependency content peek, `.test.ts` ↔ `.ts` linking). See `docs/epics/qits-lit-webcomponents/features/2026-07-26_servable-quarkus-lit-fixture.md`.
- **`qits-qits.git` / `demo-demo.git`** *(both build-derived, no submodule, no committed files)* — the **project-wrapper** fixtures (`docs/epics/qits-projects/features/2026-07-26_project-wrapper-repository.md`). A wrapper may only be adopted from a url whose basename is exactly `<slug>-<slug>`, so no existing fixture can stand in for either. `qits-qits.git` is initialized **ref-less** (`git init --bare` + `symbolic-ref HEAD refs/heads/main`) — the real `wohlben/qits-qits` starting state, a forge repo created and never pushed to; adopting it must produce the project template skeleton on `main`. `demo-demo.git` is derived from the `testing-repo` worktree (real history, project slug `demo`) and proves adoption leaves an existing history untouched. A ref-less bare has no content to commit, which is why it is created by the script rather than committed.
