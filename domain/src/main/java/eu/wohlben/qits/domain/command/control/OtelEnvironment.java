 ...
 * by workspace — the correlation backbone of the observability feature.
 *
 * <p>The endpoint host is composed exactly like {@code WorkspaceService}'s git URL: the process
 * runs inside a workspace container, so {@code localhost} would miss — {@link QitsHostResolver}
 * picks the container-reachable address per environment. SDKs append {@code /v1/<signal>} to the
 * endpoint themselves, including its {@code /api/otel} path prefix.
 */
@ApplicationScoped
