...
/**
 * The small set of read-only filesystem primitives the workspace file browser needs, decoupled from
 * <em>where</em> a workspace's files live. With workspace containers the working tree is a clone
 * inside a per-workspace Docker container ({@code /workspace}), not a host checkout, so the default
 * {@link ContainerFileAccess} implements these through {@code docker exec}. The seam keeps {@link
 * WorkspaceFilesService} pure orchestration + path-safety policy — it never touches {@code
 * java.nio.file} — so the controllers, DTOs and the whole frontend layered on it stay
 * byte-identical.
 *
 * <p>All paths are relative to the workspace root ({@code /workspace}); callers are responsible for
 * the lexical {@code ..}/absolute-escape guard before handing a user-supplied path here.
 */
public interface WorkspaceFileAccess {
  ...
  /**
   * One filesystem entry. {@code path} is workspace-root-relative; {@code size} is the byte size
   * for files (0 otherwise); {@code childCount} is the immediate-child count for directories (0
   ...
   */
  String git(String repoId, String workspaceId, String... args);
   ...
   */
  Entry stat(String repoId, String workspaceId, String path);
   ...
   */
  List<Entry> list(String repoId, String workspaceId, String dir);

  /**
   * The cheap immediate-child count of a directory (one level, never a recursive descendant walk).
   */
  int childCount(String repoId, String workspaceId, String dir);
   ...
   * to an absolute path) that points outside the workspace is rejected. A missing path, or a broken
   ...
   */
  boolean resolvesInsideRoot(String repoId, String workspaceId, String path);
   ...
   */
  byte[] read(String repoId, String workspaceId, String path);
}
