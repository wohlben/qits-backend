# qits-artifacts: a metadata-rich blob store for build artifacts

## Introduction

qits grows a **split-off artifacts**: a metadata-rich blob store for build artifacts, in its
**own Maven module** `artifacts/` (explicitly *not* in `domain` — persistence, entities, control
...
- **Sibling of the in-monolith capture store** —
  ...
  keeps a capture as workspace-lifecycle-bound goal text; artifacts is the service-shaped
...
### A new library-jar module `artifacts/`

A plain library jar like `domain` (`eu.wohlben.qits.artifacts`, BCE split `entity/ persistence/
...
(`quarkus.index-dependency.artifacts.*`), and hosts the boundary in `eu.wohlben.qits.artifacts.api`.
Module-scoped builds (`-pl artifacts`) never need the `-Dqits.variant` flag. The future standalone
deployable is a small new Quarkus-app module depending on `artifacts` — the same relationship
`service` has to `domain`.

### Own datasource, own Flyway lineage (split-deployment-ready)

Artifacts persists to its **own named datasource** — a separate file-based H2
(`~/.qits/data/artifacts/h2/artifacts`) with a **dedicated Hibernate persistence unit** and its
**own Flyway lineage** (`classpath:db/artifacts/migration`, kept clear of `domain`'s
`db/migration`). Even while in-process, artifacts never shares qits' DB or migration history, so
...
### API (all under `/api/artifacts`, hidden from OpenAPI)
...
- `PUT /api/artifacts/repositories/{repo}` — idempotently create/ensure a repository with a type
  (`{"type": "ci-screenshots"}`). Also self-seeded at startup (below).
- `GET /api/artifacts/repositories` — list.
- `POST /api/artifacts/repositories/{repo}/blobs` — raw body upload; `Content-Type` supplies the
  claimed media type, metadata rides `X-Artifacts-Meta-<key>` headers. Returns `201 {id, existing}`
  ...
  are tracked in `docs/issues/2026-07-19_artifacts-global-max-body-size-widens-public-ingest-dos.md`.
- `GET /api/artifacts/repositories/{repo}/blobs/{id}` — bytes, stored media type as `Content-Type`,
  `Cache-Control: public, max-age=31536000, immutable`. Directly usable as an `<img>`/`<video>` src.
- `GET /api/artifacts/repositories/{repo}/blobs?meta.<key>=<v>&latest=true` — metadata records,
...
- **Trust model for uploads** → a **single static API token** (`qits.artifacts.token`), checked on
  writes (POST/PUT) via the `X-Artifacts-Token` header by `ArtifactsTokenFilter` in `service`.
...
- **Who ensures the repositories exist?** → **startup self-seed**. `ArtifactsStartupSeed` (in
...
ART="http://qits:8080/api/artifacts"           # the qits-net alias; no host-port publishing
TOKEN_HEADER=(-H "X-Artifacts-Token: ${QITS_ARTIFACTS_TOKEN}")  # omit when unset (dev)
...
curl -fsS -X POST "${ART}/repositories/ci-screenshots/blobs" "${TOKEN_HEADER[@]}" \
  ...
  -H "X-Artifacts-Meta-git.branch.name: ${BRANCH}" \
  -H "X-Artifacts-Meta-git.commit.hash: ${COMMIT}" \
  -H "X-Artifacts-Meta-qits.userflow.name: ${FLOW}" \
  -H "X-Artifacts-Meta-qits.userflow.hash: ${FLOW_HASH}" \
  -H "X-Artifacts-Meta-qits.display.name: step 3 — cart" \
  -H "X-Artifacts-Meta-qits.diff.hash: $(sha256sum step3.png | cut -d' ' -f1)" \
  -H "X-Artifacts-Meta-media.resolution.width: 1440" \
  -H "X-Artifacts-Meta-media.resolution.height: 900" \
...
curl -fsS -X POST "${ART}/repositories/ci-videos/blobs" "${TOKEN_HEADER[@]}" \
  ...
  -H "X-Artifacts-Meta-qits.userflow.name: ${FLOW}" \
  -H "X-Artifacts-Meta-qits.userflow.hash: ${FLOW_HASH}" \
  -H "X-Artifacts-Meta-qits.display.name: full run" \
  -H "X-Artifacts-Meta-qits.diff.hash: ${FLOW_HASH}-${BRANCH}" \
  -H "X-Artifacts-Meta-media.resolution.length: 12" \
  --data-binary @run.webm
```

The golden query for the diff UI is then, per branch:
`GET /api/artifacts/repositories/ci-screenshots/blobs?meta.git.branch.name=main&meta.qits.userflow.name=checkout&latest=true`.

## Testing

- **`artifacts` suite** (in-memory H2, no docker, no variant flag): upload → dedupe on identical
...
- **`service` suite** (`@QuarkusTest`): upload/serve/query HTTP round-trip (201, exact bytes, stored
  ...
  a wrong token, 200 with it, serve stays open); `PublicPaths` includes `/api/artifacts/`.

## Follow-ups (create as their own epics)

Each a new repository **type** over the same core, referencing this epic as backbone:
`qits-artifacts-maven-repository`, `qits-artifacts-npm-repository`,
`qits-artifacts-docker-repository` (stub epics created alongside this feature). Already drafted
