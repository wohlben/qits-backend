...
- **Hard dependency** — [Part 1](workspace-daemon-binary-and-control-socket.md) (socket + envelope).
...
- **The initial clone** in `provisionContainer` (`:201`) — `git clone --branch <branch>
  <cloneUrl> /workspace` + `wireSubmodules` (`:255`). Once `workspace-daemon` is PID 1, the clone can
  become `workspace-daemon`'s **first job on boot** (it knows its identity + clone URL), removing the
...
- `workspace-daemon` runs them in-container; `WorkspaceService` calls the socket when a client is live,
  **falls back to `docker exec`** otherwise.
- Consider making the initial clone `workspace-daemon`-driven on boot (provisioning writes only the
  branch ref + STOPPED row, as today; `workspace-daemon` materializes `/workspace` when it starts) — a
