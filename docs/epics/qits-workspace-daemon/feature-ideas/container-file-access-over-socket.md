# Container file access over the `workspace-daemon` socket (replace `docker exec cat`/`tar`)

## Introduction

Part 3 of [qits-workspace-daemon](../epic.md). **Out of scope until
[Part 1](workspace-daemon-binary-and-control-socket.md) lands.** Moves reading and writing files in
`/workspace` off the host `docker exec` shell and onto the control socket: qits sends
`ReadFile`/`WriteFile`/`ListDir` messages and `workspace-daemon` performs the I/O in-container.
