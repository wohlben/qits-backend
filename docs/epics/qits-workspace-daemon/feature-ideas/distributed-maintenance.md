# Distributed maintenance driven through `workspace-daemon`

## Introduction

Part 7 of [qits-workspace-daemon](../epic.md). **Out of scope until
[Part 1](workspace-daemon-binary-and-control-socket.md) lands** (and after the execution/daemon parts).
Moves the periodic and housekeeping work qits performs *against* a workspace container — today
host-scheduled and executed via `docker exec` — into `workspace-daemon`, which runs it locally on socket
...
- **Hard dependency** — [Part 1](workspace-daemon-binary-and-control-socket.md); builds on the
...
- **Periodic checkpoint push** — `WorkspaceCheckpointService` (host timer,
  ...
  `docker exec git`. Becomes a `workspace-daemon` local task (ties to
...
- **Straggler reaping** — `reapStragglers()` scanning `/proc` (Part 5 already folds this into
  `workspace-daemon` as PID 1); listed here as maintenance for completeness.
- **Health probing** — `HealthProbeService` (`qits.daemons.*` poll knobs) polls in-container; can
  push from `workspace-daemon` instead of host poll.
- **Cache hygiene** — the shared maven/pnpm/claude cache volumes accumulate; `workspace-daemon` can prune
  on instruction.

## Scope

- A `Maintenance { kind }` instruction + scheduled local tasks in `workspace-daemon`, reporting outcomes
  over the socket. Move checkpoint push, health, and cache hygiene off host-scheduled
  `docker exec`.
- Preserve the existing intervals/knobs as configuration `workspace-daemon` honors (or that qits sends on
