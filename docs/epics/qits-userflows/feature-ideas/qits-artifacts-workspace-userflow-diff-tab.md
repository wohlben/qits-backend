...
evaluation**. qits' backend queries artifacts for both sides (latest story documents on
...
it is given — it never queries artifacts, never pairs, never hashes. The evaluation runs as
in-process calls into the artifacts module today (its API is hosted by `service` per the
artifacts plan's module split) behind a client seam that becomes an HTTP client after the
future deployment split.

Related/dependent plans:

- **Hard dependency** — [qits-artifacts](../../qits-artifacts/feature-ideas/qits-artifacts.md) (the store, the metadata
  contract — this tab is why `qits.display.name`/`qits.diff.hash` are mandatory there) and the
  [qits-userflows-artifacts-renderer](qits-userflows-artifacts-renderer.md) (the
...
- **Session-authed media precedent** —
  ...
  the artifacts blob reads consumed by `<img>`/`<video>` are same-origin `api/…` fetches
...
2. Queries `ci-userstories` with the `latest=true` collapse for each branch — two queries,
   server-side, through the artifacts client seam.
...
- A branch with no story documents yields an empty side (everything `NEW`, or an empty list) —
  never an error. A failing artifacts query → 5xx with a clear message (502 once artifacts
...
`api/artifacts/repositories/…/blobs/<id>` URLs (the renderer's deliberate URL form), which
resolve same-origin against the `service`-hosted artifacts API with session auth riding
...
- **Expanded**: two panes — **left = old** (parent's story document), **right = new**
  ...
  video. The markdown renderer special-cases artifacts blob links to video mediatypes into
...
- **Backend evaluation unit tests** (stubbed artifacts query seam): pairing by flow name;
...
- **Controller test**: response shape with inlined markdown; failing artifacts query ⇒ 5xx;
