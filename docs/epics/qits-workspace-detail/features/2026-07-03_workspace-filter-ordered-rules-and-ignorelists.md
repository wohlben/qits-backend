# Workspace file-tree filtering: ordered white/blacklist rules + dynamic ignorelist filters

## Introduction

Extends the advanced filter dialog of the workspace file browser
([2026-07-02_workspace-file-browser.md](2026-07-02_workspace-file-browser.md)) in two steps: the
...
**dynamic filters** are added — rule sets derived automatically from workspace content, starting
...
- Contents of *every* file with the selected basename are fetched reactively via `injectQueries`
  (dynamic count), reusing the viewer's `['workspace-file', repoId, workspaceId, path]` query key so
...
- `workspace-file-browser.component.spec.ts` — legacy `excludes` migration, mode toggle + reorder,
...
- Verified end-to-end in the running app (headless browser): added a `.gitignore` (`*.txt` +
  `!hello.txt`) to a demo workspace; the picker offered it, selecting it hid `feature.txt`/
