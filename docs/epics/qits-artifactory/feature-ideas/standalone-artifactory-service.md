# Split artifactory into its own Quarkus server

## Introduction

The [qits-artifactory backbone](../features/2026-07-19_qits-artifactory.md) was deliberately built
**split-deployment-ready**: *all* artifactory code lives in the `artifactory/` library module (no
...
the backbone was shaped for: a **new small Quarkus-app module** that depends on `artifactory` and
hosts the same API paths, so artifactory runs as its **own server** — the same relationship `service`
...
- **Directly resolves**
  [artifactory-global-max-body-size-widens-public-ingest-dos](../../../issues/2026-07-19_artifactory-global-max-body-size-widens-public-ingest-dos.md):
  today artifactory and qits share one HTTP server, and `max-body-size` is a **hard global ceiling on
  ...
  exposure modest. A separate server severs the coupling cleanly: artifactory sets a large
...
- **Backbone / boundary being lifted** — the store plan's *Design sketch* already names this: "The
  ...
  `artifactory` — the same relationship `service` has to `domain` today," and "the module boundary is
  drawn so the likely future split into a separately deployed service is a lift-and-wire, not a
  refactor." The REST paths (`/api/artifactory/…`) were chosen to "map 1:1 onto the standalone
...
- **Networking** —
  ...
  the split introduces a dedicated **`qits-artifactory`** alias on `qits-net` and its own
  compose/devcontainer service; workspace containers upload to `http://qits-artifactory:<port>/…`
...
- **Deployment** — [deployment guide](../../../guides/deployment.md), `docker-compose.prod.yml`, the
  Dokploy overlay: the split adds an artifactory service under the same self-contained-Dockerfile
  discipline (`docs/qits.../Dockerfile` gains an artifactory-app stage), and the guide gains its
  artifactory section. Its own data volume for `~/.qits/data/artifactory` (H2 + blobs).
- **Auth** — [build-variant-auth](../../../epics/qits-authentication/features/2026-07-16_build-variant-auth.md):
  ...
  writes guarded by the static `qits.artifactory.token`). A standalone server has **no
  `QitsAuthPolicy`** — the static token becomes the *whole* auth story on its own surface, so the
  `PublicPaths` entries in `auth-core` are removed once nothing hosts artifactory in `service`. The
  scoped-token direction (`docs/epics/qits-tokens/`, if pursued) is the later hardening.

## Motivation

Artifactory and qits have genuinely different HTTP profiles. qits' API is small JSON over a same-origin
SPA, with two public unauthenticated ingest endpoints (capture, OTLP) that must stay wire-size-bounded.
Artifactory is a blob server that streams large media. Forcing both through one Quarkus HTTP server
...
`ci-videos` cap). Everything else about artifactory is *already* separate (module, datasource, Flyway,
...
The split is also the point at which "artifactory is its own deployable" stops being a design claim and
...
- **New module `artifactory-service/`** (Quarkus app, `<packaging>quarkus</packaging>`) depending on
  `artifactory` + `quarkus-rest-jackson` + `quarkus-vertx-http`. It **moves** (not copies) the boundary
  classes out of `service`'s `eu.wohlben.qits.artifactory.api` package: `BlobController`,
  `RepositoryController`, `ArtifactoryExceptionMapper`, `ArtifactoryTokenFilter`,
  `ArtifactoryStartupSeed`. Its `application.properties` sets a large `max-body-size`, the artifactory
  datasource/PU/Flyway (or inherits them from the jar's `microprofile-config.properties`), and
  `quarkus.http.port`.
- **`service` drops the artifactory dependency**, its `quarkus.index-dependency.artifactory.*`, the
  `max-body-size` bump, and the `/api/artifactory` `PublicPaths` entries. `RepositoryType.maxBytes` for
  `ci-videos` returns to a natural size once the shared ceiling is gone.
- **Producers/consumers re-point** from the `qits` alias to `qits-artifactory` (the `QitsHostResolver`
  gains an artifactory host, or a dedicated resolver). The future diff UI calls the artifactory host
  directly.
- **Compose/Dokploy**: a new `qits-artifactory` service + a data volume; the guide documents it.

## Open questions

- **CORS / same-origin.** With artifactory on a different origin than the qits SPA, the future diff
  UI's `<img src="http://qits-artifactory/…">` is cross-origin. Serve responses may need permissive
...
- **One deployable or two by default?** For small/self-hosted installs, running a second server is
  overhead. Options: ship both a combined dev mode (artifactory boundary still hostable in `service`
...
- **Shared vs separate `~/.qits/data/artifactory`.** In-process today both qits and the (hypothetical)
  split write the same path; as separate containers they need the volume mounted into the artifactory
