# MCP termination in-container + token provisioning by `clientd`

## Introduction

Part 6 of [qits-workspace-client](../epic.md). **Out of scope until
[Part 1](clientd-binary-and-control-socket.md) lands** (and after
[Part 2](command-execution-over-socket.md), since agent launches ride the command path). Makes
`clientd` **terminate the workspace's MCP connection inside the container** — it is the endpoint
...
- **Hard dependency** — [Part 1](clientd-binary-and-control-socket.md); rides
...
- **Dovetails with** [qits-tokens](../../qits-tokens/epic.md) / the
  [scoped-tokens](../../qits-tokens/feature-ideas/scoped-tokens.md) draft — `clientd` is the
...
- **MCP URL composition** — `serversFor()` (`:705`) builds scoped servers with
  ...
  `clientd`, the agent connects to a **local** MCP endpoint `clientd` exposes in the container;
  `clientd` proxies/authorizes the calls back to qits over the control socket. The `qitsHost`
...
- **Credentials** — today there is **no per-session secret**: `withAgentHome()` (`:689`) points
  ...
  by `docker/workspace/agent-login.sh`. Every workspace's agent shares that identity. `clientd`
...
- `clientd` hosts the in-container MCP endpoint the agent is configured against (replacing
  ...
  handshake.
- Token provisioning: `clientd` obtains and injects the credentials the agent needs (MCP auth,
...
- Agent talks to the local `clientd` MCP endpoint; a scoped tool call round-trips to qits and is
