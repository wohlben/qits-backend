...
workspace was a **branch ref in the bare origin** plus a **container** that clones the branch into
`/workspace` over qits' in-process JGit HTTP server — but the code still treated the *container* as
the source of truth for whether a workspace existed, so a container lost for a mundane reason (daemon
restart, `docker rm`, a failed `docker run`) got the workspace marked ABANDONED even though the branch
...
- **Phase 1 — [workspace-containers](2026-07-04_workspace-containers.md)**: the `docker run` +
  clone-from-`/git` lifecycle, `Workspace.branch` as a stored column, container-label reconciliation.
- **Phase 2 — [container-file-access](2026-07-04_container-file-access.md)** and **Phase 3 —
  [container-agent-sessions](../../qits-coding-agents/features/2026-07-04_container-agent-sessions.md)**: their entry points inherited
  the "container missing → hard fail" problem this fixes (now they lazily re-provision).
- Reworks reconciliation from [repository-discovery](../../qits-project-repositories/features/2026-05-01_repository-discovery.md) and the
  ACTIVE/ABANDONED lifecycle from [workspace-history](2026-06-30_workspace-history.md).
- **Next step (now done)**: the worktree→workspace **rename** (§G of the original idea) is captured as
  [worktree-to-workspace-rename](2026-07-05_worktree-to-workspace-rename.md) — the
  follow-up that was kept out of this behaviour diff on purpose.
...
`WorkspaceRuntimeStatus { RUNNING, STOPPED, PROVISIONING, FAILED }`, a persisted `runtime_status`
column (+ `runtime_error`) on `Workspace` (Flyway `V21`), distinct from the lifecycle `WorkspaceStatus`.
`WorkspaceService.listWorkspaces` computes `RUNNING` live from the container listing (one `docker ps`)
...
### `ensureContainer(repoId, workspaceId)` (§A)

`WorkspaceService.ensureContainer` — idempotent provisioning:
...
- container absent but the branch ref survives in origin → re-materialize from the branch (the
  existing `createContainerWorkspace(…, createBranchRef=false)` path `createMainWorkspace` uses).
- branch ref gone from origin → the work no longer exists anywhere: the workspace is ABANDONED here —
...
through `prepare`), the file browser (`WorkspaceFilesService`), the workspace-local git verbs
(`fastForwardWorkspace`, `updateWorkspaceFromParent`), and the agent chat sign-in probe.
...
- `stopContainer()` — a lossless graceful stop: pushes the branch to origin first (`pushBranch`), then
  removes the container, leaving the workspace ACTIVE/`STOPPED` for lazy recreate.
...
The dead `Path.of(dataDir, repoId, "workspaces", workspaceId)` host-checkout probes are gone:
`CommitService` (incoming-commits branch + parent resolution) and `ResolveConflictService` read the
stored `Workspace.branch` column; `DaemonSupervisor` tails log sources **inside the container** via a
...
`PromptRefinementService` now runs `claude` **inside the workspace container**
...
Endpoints `POST …/workspaces/{id}/ensure-container` (start/recreate) and `…/stop-container` (graceful
stop), each returning the refreshed `WorkspaceDto`. The branch row shows a runtime-status badge (with
...
- **§G rename** done in [worktree-to-workspace-rename](2026-07-05_worktree-to-workspace-rename.md).
...
- **Unit (no docker)** — `WorkspaceContainerLifecycleServiceTest`: recreate-from-branch, no-op when
...
- **Controller** — `WorkspaceControllerTest`: the ensure/stop endpoints round-trip the runtime status.
...
- **Real docker (extended)** — two `@Tag("extended")` ITs, opt in with `./mvnw verify -Pextended`
  ...
  - `WorkspaceRecreateIT` — the **full end-to-end recreate round-trip** (`@QuarkusIntegrationTest`, so
    the real `DockerExecutor` runs, not the fake): create a repo + workspace over REST, push a commit
