# Run Action in Worktree

## User Story

As a developer, I want to run a standardized action inside a worktree and interact with it live, so that routine tasks (shells, builds, dev servers) happen in the isolated change context instead of on my machine.
...
1. On a worktree row in the repository's branch tree, choose **Run…**.
...
- `commands` — launch an action as a command in a worktree
