...
A repository is a version-controlled codebase that serves as the technical substrate for change work. It enables the project, workspace, and feature-flow domains by providing the code that moves through a pipeline.
...
The center of the repository page is the **branch tree**: branches and their workspaces rendered as a forest, each branch nested under the parent it was forked from. From here users inspect commit history, see how far a change has drifted from its parent, and drive the whole workspace lifecycle (see the workspace domain).

Repositories on disk are the source of truth at startup: QITS auto-discovers repositories and workspaces found in its data directory and reconciles them with its database, using JSON metadata files kept alongside each repository.
