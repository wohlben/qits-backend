...
Because worktrees are resolved rather than deleted, each one remains a durable unit-of-work record: why it existed (preamble), how it ended (result), what happened along the way (event timeline), and what ran inside it (commands and agent sessions).
...
2. The history lists all worktrees — active and resolved — with status, parent, and created/resolved dates.
...
- `repositories-repoId-history` — list the repository's worktree history
- `repositories-repoId-history-id` — one worktree's full record (narrative, timeline, commands); also updates the narrative
