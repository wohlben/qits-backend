...
When proposing the change, the developer also states its **intent**: the creation dialog takes a worktree ID, the new branch name, and a markdown **goal (preamble)** describing why the work exists and what "done" means. The preamble stays editable while the worktree is active, is shown wherever the worktree is worked on (e.g. the work-in-progress page), and becomes part of the permanent record once the worktree is resolved.
...
- `repositories-repoId-worktrees` — create the worktree (records a `created` event)
