...
| `docker-compose.dokploy.yml` | Overlay for [Dokploy](https://dokploy.com)-managed deployments: `extends` the qits service from the prod file, adds a `workspace-image` build service wired via `additional_contexts` as the app build's base (fully-from-source — no manual image prep, no Docker Hub pull; compose v2.22+), and joins `dokploy-network` so Dokploy's Traefik can route to it. Full walkthrough in the README's "Deploying with Dokploy" section. |
