...
The consumer that closes the [qits-artifactory](qits-artifactory.md) →
...
- **Hard dependency** — [qits-artifactory](qits-artifactory.md) (the store, the metadata
...
- **Producer** — [qits-userflows](qits-userflows.md) stories, run via
  [actions](../features/2026-05-01_actions.md) on the parent branch once and on the workspace
...
- **One more `<z-tab>`** in the group owned by
  ...
  [draggable tabs](../features/2026-07-09_draggable-workspace-detail-tabs.md) absorbs the new
  label, and [tab-url deep links](../features/2026-07-10_workspace-tab-url-and-picked-file-deep-link.md)
  need a `CI → ci` slug entry.
- **Markdown rendering precedent** —
  [chat-markdown-rendering](../features/2026-07-02_chat-markdown-rendering.md): the story
  document is markdown; the tab reuses the webui's markdown rendering path rather than
  growing a second one.
- **Session-authed media precedent** —
  [capture-rendered-view-screenshot](capture-rendered-view-screenshot.md)'s screenshot GET:
...
- **Live freshness convention** — one initial fetch per activation plus explicit refresh;
  ...
  [workspace SSE channel](../features/2026-07-07_workspace-sse-live-updates.md), never a poll
  interval.
