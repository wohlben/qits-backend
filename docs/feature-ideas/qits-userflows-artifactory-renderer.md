# qits-userflows-artifactory-renderer: publish user-story reports into the artifactory

## Introduction

The piece that finally combines the [qits-artifactory](qits-artifactory.md) and
...
screenshots and video to their artifactory repositories (obtaining content-addressed blob
URLs), then re-renders the story markdown *against those uploaded URLs*, and finally stores
the story document itself as a blob — which requires **extending artifactory with a third
...
artifactory: the story documents the
[user-flow diff tab](qits-artifactory-workspace-userflow-diff-tab.md) compares
*and* a self-contained story document whose images/video resolve wherever the artifactory API
is reachable.
...
- **Hard dependency (store side)** — [qits-artifactory](qits-artifactory.md): uses the upload
...
- **Completes the loop for** the
  [user-flow diff tab](qits-artifactory-workspace-userflow-diff-tab.md): the `ci-userstories`
...
- **Runs as a workspace action** — [actions](../features/2026-05-01_actions.md) /
  [feature-flows](../features/2026-05-01_feature-flows.md): branch/commit context enters here
...
Added to artifactory's type roster (same validation-profile mechanism, no core changes):
...
- Blobs are the **rendered story markdown**: description, step block, and media references as
  base-relative artifactory URLs (below). Content-addressing works unchanged — markdown is
  bytes like everything else.

## Renderer contract

Input: one story directory's `userflow.json` (+ the media files it references). Config: the
artifactory base URL and, when the write surface grows tokens, the credential — both from
...
3. **Ensure repositories** with artifactory's idempotent `PUT` (`ci-screenshots`, `ci-videos`,
...
4. **Upload media first** (the URLs are needed before the document can render):
   ...
   - the video → `ci-videos`, same shared keys; its **`qits.diff.hash` = a digest over the
     ...
     digest" candidate from the artifactory plan, adopted here as the provisional v1 scheme
...
5. **Render the story markdown against the uploaded URLs**: the same document shape as the
   ...
   (`api/artifactory/repositories/ci-screenshots/blobs/<id>`) instead of a local filename.
...
- **Code lives in the userflows module's `src/main`** (`…userflows.render.artifactory`), as
...
- **URL form**: base-relative (`api/…`, no leading slash) per the webui convention, matching
  ...
  qits surface; a non-qits consumer prefixes the artifactory origin itself.

## Out of scope

- **Displaying `ci-userstories` documents** — that is the
  [user-flow diff tab](qits-artifactory-workspace-userflow-diff-tab.md)'s job; this renderer
  ends at the upload.
- Retention/pruning of story documents (rides the artifactory plan's open retention
  question).
...
- **Video diff-hash v1**: the screenshot-trail digest means a visual video change with no
  ...
  that care add a screenshot step); revisit when the artifactory plan's video-hash question
  settles for real.
...
- **Credential plumbing**: depends on where the artifactory write-surface question lands
...
- **Renderer unit tests** (userflows module, against a stub HTTP server): upload order
  ...
  reported.
- **Artifactory side** (artifactory + service suites): `ci-userstories` type profile —
