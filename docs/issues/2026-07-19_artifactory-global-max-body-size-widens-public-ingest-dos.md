# Artifactory's global max-body-size raise widens the public capture/OTLP ingest DoS surface

## Introduction

Introduced by [qits-artifactory](../epics/qits-artifactory/features/2026-07-19_qits-artifactory.md).
...
Artifactory must accept media uploads larger than Quarkus' 10 MB default `max-body-size`. That
...
Consequently the only way to accept large artifactory uploads is to raise the **global** limit, which
...
- The upload stays a JAX-RS `InputStream` resource (`BlobController.upload`), which streams to disk
  incrementally — it does not buffer the whole body in memory (unlike a `BodyHandler`), so artifactory
...
raised freely for artifactory without exposing capture/OTLP. Deferred because it modifies the request
handling of endpoints owned by other features (capture, telemetry) and wants their own regression
coverage; it is out of the artifactory feature's scope.
...
artifactory upload of the same size succeeds — proving the limits are decoupled.
