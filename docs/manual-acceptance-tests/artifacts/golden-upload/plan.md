# Acceptance: artifacts golden upload + query round-trip

## Introduction

Accepts the **[qits-artifacts](../../../epics/qits-artifacts/features/2026-07-19_qits-artifacts.md)**
...
automated `artifacts` + `service` suites (which never touch docker or a real container) by proving
...
- If `qits.artifacts.token` is set on this deployment, export it into the workspace as
  `QITS_ARTIFACTS_TOKEN` (the devcontainer/dev default is blank → the guard is a no-op).
...
1. **Confirm the default repositories self-seeded.** From the qits host/devcontainer terminal:
   ```bash
   curl -fsS localhost:8080/api/artifacts/repositories | jq
   ```
...
2. **Open a shell inside a workspace container** (materialize one via the UI or reuse the seed-webapp
   ...
   ```bash
   curl -fsS http://qits:8080/api/artifacts/repositories >/dev/null && echo reachable
   ```
...
3. **Upload a real screenshot** from inside the container (produce one with any tool, e.g. a
   ...
   ```bash
   curl -fsS -X POST http://qits:8080/api/artifacts/repositories/ci-screenshots/blobs \
     ${QITS_ARTIFACTS_TOKEN:+-H "X-Artifacts-Token: ${QITS_ARTIFACTS_TOKEN}"} \
     -H "Content-Type: image/png" \
     -H "X-Artifacts-Meta-git.branch.name: $(git branch --show-current)" \
     -H "X-Artifacts-Meta-git.commit.hash: $(git rev-parse HEAD)" \
     -H "X-Artifacts-Meta-qits.userflow.name: checkout" \
     -H "X-Artifacts-Meta-qits.userflow.hash: $(sha256sum tests/checkout.spec.ts | cut -d' ' -f1)" \
     -H "X-Artifacts-Meta-qits.display.name: step 3 — cart" \
     -H "X-Artifacts-Meta-qits.diff.hash: $(sha256sum shot.png | cut -d' ' -f1)" \
...
4. **Upload a real video** (`run.webm`):
   ```bash
   curl -fsS -X POST http://qits:8080/api/artifacts/repositories/ci-videos/blobs \
     ${QITS_ARTIFACTS_TOKEN:+-H "X-Artifacts-Token: ${QITS_ARTIFACTS_TOKEN}"} \
     ...
     -H "X-Artifacts-Meta-qits.diff.hash: run-$(git branch --show-current)" \
...
5. **Query the goldens for this branch** (from container or host):
   ...
   curl -fsS "http://qits:8080/api/artifacts/repositories/ci-screenshots/blobs?meta.git.branch.name=${BRANCH}&meta.qits.userflow.name=checkout&latest=true" | jq
   curl -fsS "http://qits:8080/api/artifacts/repositories/ci-videos/blobs?meta.git.branch.name=${BRANCH}&meta.qits.userflow.name=checkout&latest=true" | jq
   ```
...
6. **Serve a blob and confirm bytes + headers.** Take the screenshot `id` from step 3:
   ```bash
   curl -fsS -D- -o served.png "localhost:8080/api/artifacts/repositories/ci-screenshots/blobs/<id>"
...
7. **Reject probes.**
   - `curl -s -o /dev/null -w '%{http_code}\n' localhost:8080/api/artifacts/repositories/ci-screenshots/blobs/not-a-sha`
     *Expect:* `404` (malformed id — path-traversal defence).
   - Upload a `video/mp4` body to `ci-screenshots` *Expect:* `400` (disallowed media type).
   - If a token is configured, POST without `X-Artifacts-Token` *Expect:* `401`.

## Acceptance checklist

- [ ] `ci-screenshots` and `ci-videos` exist at boot without any manual PUT.
- [ ] A workspace container uploads a screenshot and a video through `http://qits:8080/api/artifacts/…`.
...
`rm -rf ~/.qits/data/artifacts` (drops the artifacts H2 + blobs; the repositories self-seed again
