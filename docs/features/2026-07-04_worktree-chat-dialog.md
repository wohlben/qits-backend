# Worktree detail: persistent chat session in a full-size dialog

## Introduction

The worktree detail route (the file browser) currently links away to the WIP route ("Speak a
...
Which chat "belongs" to this worktree must survive dialog close, route re-entry, and page
reload — so it can't live in the dialog component:

- `CommandDto` already carries `worktreeId`, `kind` and `status`. The dialog's owning
  component resolves the current session as *the most recent `CHAT` command for this worktree
...
- If multiple running chats exist for one worktree (possible via the Commands page), pick the
...
- **Extract, don't duplicate**: the WIP page's body (preamble + `SpeakToPromptComponent`) is
  ...
  transition.
- The dialog is opened from the worktree-detail page via `ZardDialogService` with an inline
...
- Worktree-detail page spec: button opens the dialog; with a mocked running `CHAT` command for
  this worktree the chat component is composed with that id; without one, speak-to-prompt is
  shown; running-session indicator reflects the commands query.
- Session-resolution util/spec: newest-running-chat-for-worktree selection (multiple chats,
  finished chats excluded, other worktrees' chats excluded).
