...
- Operates on the tree built by `shared/utils/build-file-tree.ts`
  ([2026-07-02_worktree-file-browser.md](2026-07-02_worktree-file-browser.md)).
- Interacts with the filter pipeline, including the planned ordered white/blacklist rules and
  dynamic ignorelist filters
  ([worktree-filter-ignorelists.md](../feature-ideas/worktree-filter-ignorelists.md)) — every one
  of those features changes the visible path set, and each change re-derives the compaction.
- Applied to both consumers of `buildFileTree`: the worktree file browser and the commit-diff
  changed-files tree ([worktree-history](2026-06-30_worktree-history.md) area).
...
- The worktree browser additionally **mirrors** a compacted node's open/closed state onto the
...
- `chains`, if provided, collects one `CompactedChain` per compaction — this feeds the worktree
...
- Called explicitly by the consumers' `computed()`s rather than folded into `buildFileTree`,
  ...
  - `pattern/worktree/worktree-file-browser.component.ts` — a `compaction` computed returns
...
- `pattern/worktree/worktree-file-browser.component.spec.ts`: compacted labels in the rendered
